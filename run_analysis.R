...
runA <- function() {
        ...
        # step 0.1 load data
                f1 <- "E:/mooc/excercise/3_cleaningdata/project/UCI HAR Dataset/features.txt"
                d_feature = read.table(f1, sep=" ")
                
                f2 <- "E:/mooc/excercise/3_cleaningdata/project/UCI HAR Dataset/train/y_train.txt"
                d_ytrain = read.table(f2, sep=" ")

                f3 <- "E:/mooc/excercise/3_cleaningdata/project/UCI HAR Dataset/train/subject_train.txt"
                d_subjecttrain = read.table(f3, sep=" ")
                
                f4 <- "E:/mooc/excercise/3_cleaningdata/project/UCI HAR Dataset/train/X_train.txt"
                d_xtrain = read.table(f4, as.is= TRUE)

                f5 <- "E:/mooc/excercise/3_cleaningdata/project/UCI HAR Dataset/activity_labels.txt"
                d_activitylabel = read.table(f5, sep=" ", na.strings = "NA", fill = TRUE)

                f6 <- "E:/mooc/excercise/3_cleaningdata/project/UCI HAR Dataset/test/X_test.txt"
                d_xtest = read.table(f6, as.is= TRUE)

                f7 <- "E:/mooc/excercise/3_cleaningdata/project/UCI HAR Dataset/test/y_test.txt"
                d_ytest = read.table(f7, sep=" ", na.strings = "NA", fill = TRUE)
                
                f8 <- "E:/mooc/excercise/3_cleaningdata/project/UCI HAR Dataset/test/subject_test.txt"
...
#        requiredColumnsList <- grep("mean",d_feature[[2]], ignore.case = TRUE)
#        print(length(requiredColumnsList))
#        requiredColumnsList <- grep("std",d_feature[[2]], ignore.case = TRUE)
#        print(length(requiredColumnsList))
